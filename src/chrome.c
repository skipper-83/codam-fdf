...
/*   Updated: 2023/02/24 23:30:14 by albertvanan      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	header_background(t_meta *m, int x_pos, int y_pos)
{
	...
	if (m->strings == NULL)
		exit_error(ERROR_MEM, m);
	item = ft_lstnew(mlx_new_image(m->mlx, 330, 310));
	if (item == NULL)
		exit_error(ERROR_MEM, m);
	if (item->content == NULL)
		exit_error(ERROR_MLX, m);
	ft_lstadd_front(&m->strings, item);
	ft_memset(((mlx_image_t *)item->content)->pixels, 0xFFFFFF44, \
												330 * 310 * sizeof(int));
	mlx_image_to_window(m->mlx, ((mlx_image_t *)item->content), \
													x_pos - 10, y_pos - 8);
	mlx_set_instance_depth(&((mlx_image_t *)item->content)->instances[0], 0);
}
...
static void	make_lines(t_meta *m, const char *chrome[13])
{
	...
	while (i < 13)
	{
		item = ft_lstnew(mlx_put_string(m->mlx, chrome[i], x_pos, y_pos + 30));
		if (item == NULL)
			exit_error(ERROR_MEM, m);
		if (item->content == NULL)
			exit_error(ERROR_MLX, m);
